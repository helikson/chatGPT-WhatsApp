...
type Choice struct {
	...
	Message struct { // Dá para usar struct Message tbm
		Role    string `json:"role"`
		Content string `json:"content"`
	} `json:"message"`
}
...
func GenerateGPTText(query string) (string, error) {
	...
	request.Header.Set("Authorization", "Bearer API_KEY_GPT")
	...
	// defer => Roda tudo que está em baixo e depois roda o defer
	// (fechando conexão nesse exemplo)
